...
export const renderBudgets = (container) => {
  ...
  container.innerHTML = `
    ...
    <div id="budgetModal" class="modal-overlay">
      <div class="modal-content card" style="max-width: 450px; width: 100%;">
        <h3 style="margin-top: 0; border-bottom: 2px solid rgba(123, 44, 191, 0.2); padding-bottom: 10px; color: var(--text-primary);">Crear Presupuesto</h3>
        <form id="budgetForm" style="display: flex; flex-direction: column; gap: 15px; margin-top: 15px;">
          ...
          <div class="form-group">
            ...
            <input type="text" id="budgetCategoria" required placeholder="Ej. Comida" class="form-input" style="width: 100%; padding: 10px; background: rgba(0,0,0,0.2); border: 1px solid rgba(255,255,255,0.1); border-radius: 8px; color: var(--text-primary); margin-top: 5px;">
          </div>
          <div class="form-group">
            <label style="color: var(--text-secondary); font-size: 0.9rem;">Monto Límite</label>
            <input type="text" id="budgetLimit" required placeholder="Ej. 500.000" class="form-input" style="width: 100%; padding: 10px; background: rgba(0,0,0,0.2); border: 1px solid rgba(255,255,255,0.1); border-radius: 8px; color: var(--text-primary); margin-top: 5px;">
          </div>
          <div style="display: flex; gap: 10px;">
            <div class="form-group" style="flex: 1;">
              <label style="color: var(--text-secondary); font-size: 0.9rem;">Fecha Inicio</label>
              <input type="date" id="budgetStart" required class="form-input" style="width: 100%; padding: 10px; background: rgba(0,0,0,0.2); border: 1px solid rgba(255,255,255,0.1); border-radius: 8px; color: var(--text-primary); margin-top: 5px;">
            </div>
            <div class="form-group" style="flex: 1;">
              <label style="color: var(--text-secondary); font-size: 0.9rem;">Fecha Fin</label>
              <input type="date" id="budgetEnd" required class="form-input" style="width: 100%; padding: 10px; background: rgba(0,0,0,0.2); border: 1px solid rgba(255,255,255,0.1); border-radius: 8px; color: var(--text-primary); margin-top: 5px;">
            </div>
          </div>
          <div class="form-group">
            <label style="color: var(--text-secondary); font-size: 0.9rem;">Ciclo de Repetición</label>
            <select id="budgetRecurrence" class="form-input" style="width: 100%; padding: 10px; background: rgba(0,0,0,0.2); border: 1px solid rgba(255,255,255,0.1); border-radius: 8px; color: var(--text-primary); margin-top: 5px;">
      ...
      <div class="modal-content card" style="max-width: 450px; width: 100%;">
        ...
        <form id="goalForm" style="display: flex; flex-direction: column; gap: 15px; margin-top: 15px;">
          <div class="form-group">
            ...
            <input type="text" id="goalName" required placeholder="Ej. Fondo de Emergencia" class="form-input" style="width: 100%; padding: 10px; background: rgba(0,0,0,0.2); border: 1px solid rgba(255,255,255,0.1); border-radius: 8px; color: var(--text-primary); margin-top: 5px;">
          </div>
          <div class="form-group">
            <label style="color: var(--text-secondary); font-size: 0.9rem;">Monto Objetivo</label>
            <input type="number" id="goalTarget" required placeholder="Ej. 5000000" class="form-input" style="width: 100%; padding: 10px; background: rgba(0,0,0,0.2); border: 1px solid rgba(255,255,255,0.1); border-radius: 8px; color: var(--text-primary); margin-top: 5px;">
          </div>
          <div class="form-group">
            <label style="color: var(--text-secondary); font-size: 0.9rem;">Cuenta Vinculada (Recibe depósitos)</label>
            <select id="goalAccount" class="form-input" style="width: 100%; padding: 10px; background: rgba(0,0,0,0.2); border: 1px solid rgba(255,255,255,0.1); border-radius: 8px; color: var(--text-primary); margin-top: 5px;">
  ...
  const renderBudgetsList = () => {
    ...
    budgetsArray.forEach(b => {
      ...
      if (progress > 85 && !isExceeded) colorClass = 'bg-warning';
      if (isExceeded) {
        ...
        <div class="budget-item" style="background: rgba(255,255,255,0.02); padding: 15px; border-radius: 12px; border: 1px solid rgba(255,255,255,0.05);">
          <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 5px;">
            ...
            <div style="display: flex; align-items: center; gap: 10px;">
              <span style="font-size: 0.8rem; color: var(--text-secondary);">${vigenciaText}</span>
              <button onclick="window.editBudget('${b.id}')" style="background: transparent; border: none; cursor: pointer; color: #4cc9f0;" title="Editar">✏️</button>
              <button onclick="window.deleteBudget('${b.id}')" style="background: transparent; border: none; cursor: pointer; color: #f72585;" title="Eliminar">🗑️</button>
            </div>
          </div>
          ...
          <div style="width: 100%; height: 8px; background: rgba(0,0,0,0.3); border-radius: 999px; overflow: hidden;">
    ...
    goals.forEach(goal => {
      ...
      html += `
        <div style="background: rgba(255,255,255,0.02); padding: 15px; border-radius: 12px; border: 1px solid rgba(255,255,255,0.05);">
          ...
          <div style="width: 100%; height: 10px; background: rgba(0,0,0,0.3); border-radius: 999px; overflow: hidden; margin-bottom: 8px;">
  ...
  const populateAccounts = () => {
    ...
    methods.forEach(m => {
      goalAccountSelect.innerHTML += `<option value="${m.id}">${m.platform || m.bank} (${m.type})</option>`;
    });
  };
  ...
  document.getElementById('goalForm').addEventListener('submit', (e) => {
    ...
    const monto_objetivo = parseFloat(document.getElementById('goalTarget').value);
